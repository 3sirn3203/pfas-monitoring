...
export default function LabSearch() {
    ...
    const addSpecies = (speciesId: string) => {
                            ...
                            {siteResults.map(s => (
                                <li key={String(s.id)}>
                                    <button
                                        ...
                                        onMouseDown={() => addSite(String(s.id))}
                                    >
                                        {String(s.id)} • {s.name}
                                    </button>
                                </li>
                        ...
                        <ul className="mt-2 max-h-40 overflow-auto rounded-md border text-sm bg-white z-10 absolute w-full">
                            {speciesResults.map(s => (
                                <li key={String(s.id)}>
                                    <button
                                        className="w-full px-3 py-2 text-left hover:bg-slate-50"
                                        onMouseDown={() => addSpecies(String(s.id))}
                                    >
                                        {s.code} ({String(s.id)})
                                    </button>
                                </li>
                    ...
                    <div className="space-y-2 max-h-40 overflow-auto">
                        {filteredRecords.map(record => (
                            <button
                                ...
                                className={`w-full p-3 text-left border rounded-lg transition-colors text-sm ${selectedRecord === String(record.id)
